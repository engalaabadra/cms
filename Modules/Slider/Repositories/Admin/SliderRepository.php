<?php
namespace Modules\Slider\Repositories\Admin;

use App\Repositories\EloquentRepository;
use Illuminate\Support\Arr;
use Modules\Slider\Repositories\Admin\SliderRepositoryInterface;
 ...
 public function store($request,$model){
            ...
            if(!empty($data['image'])){
                if($request->hasFile('image')){
                   $path= $this->storeImage($request,'image','sliders-images');
                                                       $item->image()->create(['url'=>$path,'imageable_id'=>$item->id,'imageable_type'=>'Modules\Slider\Entities\Slider']);
        ...
        public function update($request,$id,$model){
            ...
            if(!empty($data['image'])){
                if($request->hasFile('image')){
                   $path= $this->storeImage($request,'image','sliders-images');
                    if($item->image){
                        $item->image()->update(['url'=>$path,'imageable_id'=>$item->id,'imageable_type'=>'Modules\Slider\Entities\Slider']);
                    }else{
                        $item->image()->create(['url'=>$path,'imageable_id'=>$item->id,'imageable_type'=>'Modules\Slider\Entities\Slider']);
                    }
                }else{
